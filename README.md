...
**2. Course database.** `data/yale_som.db` is included in the repo, so there's nothing to download. The catalog cards load from `data/yale_som_classes.json`, and the agent's `search_courses` tool reads the database.
